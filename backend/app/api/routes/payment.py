...
from fastapi import (
    Header, APIRouter, Response, Depends, HTTPException, status, Query, Body, BackgroundTasks
)
from app.auth.openapi_responses import FORBIDDEN_RESPONSE, UNAUTHORIZED_RESPONSE
from app.email_utils import generate_product_email_template, send_email
from app.payment.openapi_responses import PAYMENT_NOT_FOUND_RESPONSE
from app.payment.schemas import (
    PaymentInfo, PaymentResponse, Payment, WalletData, TipsInfo, CancelList
)
from app.auth.deps import get_current_active_user
from app.database.redis import get_redis_connection
from app.payment.utils import verify_payment_hook, check_on_data_relevance, prepare_data_to_redis_storage, \
    decode_data_from_redis_storage
from app.payment.dal import PaymentDAL, PaymentInfoDal, WalletDataDal, TipsInfoDal, CancelListDal
...
async def payment_hook(
        ...
        payment_info_dal: Annotated[PaymentInfoDal, Depends(PaymentInfoDal.get_as_dependency)],
    ...
    if verify_payment_hook(x_sign_base64, body):
        ...
        if stored_payment:
            stored_modified_date = decode_data_from_redis_storage(stored_payment)["modifiedDate"]
            requested_modified_date = body.get("modifiedDate")
            if not check_on_data_relevance(requested_modified_date, stored_modified_date):
        ...
        cancel_list = CancelList(**body["cancelList"]).model_dump(exclude_unset=True) \
            if body.get("cancelList") else None
        wallet_data = WalletData(**body["walletData"]).model_dump(exclude_unset=True) \
            if body.get("walletData") else None
        tips_info = TipsInfo(**body["tipsInfo"]).model_dump(exclude_unset=True) \
            if body.get("tipsInfo") else None
        payment_info = PaymentInfo(**body["paymentInfo"]).model_dump(exclude_unset=True) \
            if body.get("paymentInfo") else None

        if payment["status"] == "success":
            new_payment = await payment_dal.create(
                payment
            )
            if cancel_list:
                await cancel_list_dal.create(
                    new_payment.id,
                    cancel_list
                )

            if wallet_data:
                await wallet_data_dal.create(
                    new_payment.id,
                    wallet_data
                )
            if tips_info:
                await tips_info_dal.create(
                    new_payment.id,
                    tips_info
                )
            if payment_info:
                await payment_info_dal.create(
                    new_payment.id,
                    payment_info
                )
...
async def get_all_payments(
    ...
    payments_invoice_stored_in_postgres = [payment.invoice for payment in payments_from_postgres]
    payments_invoice_stored_in_redis = await redis.keys("*")
    uncompleted_payments_invoice = set(payments_invoice_stored_in_redis) - set(payments_invoice_stored_in_postgres)
